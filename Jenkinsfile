            ...
            timeout(time: 15, unit: 'MINUTES') {
                ...
                try {
                    ...
                    dir('integrationTests') {
                        ...
                        docker.image('node:9-stretch').inside("-e WEBDRIVER=remote -e CES_FQDN=${cesIP} -e SELENIUM_BROWSER=chrome -e SELENIUM_REMOTE_URL=http://${seleniumChromeIP}:4444/wd/hub") {
                            sh 'yarn install'
                            sh 'yarn run ci-test'
                        }
                    }
